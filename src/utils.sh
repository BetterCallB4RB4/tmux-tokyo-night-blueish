...
function generate_active_window_string() {
	...
	separator_start="#[bg=${PALLETE['purple']},fg=${PALLETE['bg_highlight']}]${left_separator:?}#[none]"
	separator_internal="#[bg=${PALLETE['purple']},fg=${PALLETE['purple']}]${left_separator:?}#[none]"
	separator_end="#[bg=${PALLETE[bg_highlight]},fg=${PALLETE['purple']}]${left_separator:?}#[none]"

	echo "${separator_start} #[fg=${PALLETE[fg_gutter]}]#I${separator_internal}#[fg=${PALLETE[fg_gutter]}] #{?window_zoomed_flag,$zoomed_window_icon,$active_window_icon}#W #{?pane_synchronized,$pane_synchronized_icon, }${separator_end}#[none]"
}
